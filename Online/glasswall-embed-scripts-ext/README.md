**Glasswall Embed Scripts - SharePoint Framework(SPFx) Extension**

# Overview

This solution contains SPFx Extension of Application Customizer type. It dynamically adds custom JS file to the site which will be used to hide the default Download button for SharePoint online (Office365) Site. As of now, the custom JS file is hosted on SharePoint site document library but later can be hosted at any place/CDN. The current code inside custom JS file requires jQuery to be loaded first which is why the extension loads jQuery first and then load custom JS file. 

## Benefits of the approach
- It allows to change JS code without re-deployment of SPFx Extension. There will not be any changes required on Office365 environment for making any update. 
- You can download the custom JS file from hosted location, update as per your requirement and upload it back.

## Drawbacks of the approach
- It takes considerate time to load the custom JS file on browser and then execute the function when document is ready. 
- This could open a loophole that user may click on default download button before it gets hidden which breaks the overall objective.
...
- Open 'spcloud-modern-customisation\glasswall-embed-scripts-ext\config\serve.json' file in VS Code or any other editor.
...
- Navigate to solution folder 'glasswall-embed-scripts-ext'.
- Run 'npm install'.
...
- This will create SharePoint Package File at location '{your-local-development-folder}.\spcloud-modern-customisation\glasswall-embed-scripts-ext\sharepoint\solution\glasswall-embed-scripts-ext.sppkg'.
...
- Open the Release 'Glasswall-Office365-Embed-Scripts-CI-Release' on DevOps (tbd).
- Click on Edit.
...
- Open the Pipeline 'Glasswall-Office365-Embed-Scripts-CI' on DevOps (tbd).
